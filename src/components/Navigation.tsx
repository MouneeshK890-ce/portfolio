      ...
      <div className="container mx-auto px-4 py-4">
        ...
        {isOpen && (
          <motion.div
            ...
            className="md:hidden mt-4 pb-4 bg-background/90 rounded-xl shadow-lg"
          >
            {navItems.map((item) => (
              <button
                key={item.name}
                onClick={() => scrollToSection(item.href)}
                className="block w-full text-left py-2 px-4 hover:bg-muted rounded-lg transition-smooth "
              >
                {item.name}
              </button>
            ))}
